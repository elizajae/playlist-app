from flask import Flask, redirect, render_template, flash, abort
...
app = Flask(__name__)
app.config['SQLALCHEMY_DATABASE_URI'] = 'postgresql://postgres:postgres@localhost/playlist-app'
...
def add_song_to_playlist(playlist_id):
    ...
    cur_on_playlist = [s.id for s in playlist.songs]
    song_choices = (db.session.query(Song.id, Song.title)
                    .filter(Song.id.notin_(cur_on_playlist))
                    .all())
    
    form.song.choices = [(song.id, song.title) for song in song_choices]
